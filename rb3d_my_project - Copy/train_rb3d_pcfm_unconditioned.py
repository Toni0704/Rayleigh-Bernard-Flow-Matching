#!/usr/bin/env python3
"""
train_rb3d_pcfm_unconditioned.py
================================================================================
Train the UNCONDITIONED flow-matching model on the RB3D multi-solution
training split -- the faithful analog of the Bratu/2D "single UNCONDITIONED
model" ablation. Learns the MARGINAL p(u,v,w,T') over ALL branches and ALL
(Ra,Pr) mixed together; the PDE parameters enter ONLY at sampling time,
through the pseudo-time physics relaxation at the target (Ra,Pr), which both
fixes the parameters and snaps the free sample onto a valid steady branch.

Run (Kaggle T4):
    python train_rb3d_pcfm_unconditioned.py \
        --bank ./datasets/rb3d_multisolution/splits/train_bank.pt
    python train_rb3d_pcfm_unconditioned.py --quick     # smoke test
...
def parse_args():
    ...
    p.add_argument('--ckpt-name', default='ckpt_rb3d_uncond.pt')
...
def main():
    ...
    model, info = train_flow_model(data, cfg, cond=False, device=device,
                                   ckpt_path=ckpt_path,
